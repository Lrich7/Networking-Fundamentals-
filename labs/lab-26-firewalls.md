...
Apply the concepts from Lesson 26 in a safe, isolated training environment and practice security-focused troubleshooting.
...
Use Windows Defender Firewall only for safe inspection/testing and Packet Tracer for network-policy practice. Inspect firewall profiles with `Get-NetFirewallProfile` and a limited set of rules with `Get-NetFirewallRule`. In Packet Tracer, create a policy scenario where a server should accept required web traffic while unnecessary access is restricted. Never disable a production firewall as a troubleshooting shortcut.
...
You have completed **Lab 26 — Firewalls**.

# Next Lesson

➡️ **[Lesson 27 — Access Control Lists](../lessons/lesson-27-access-control-lists.md)**
